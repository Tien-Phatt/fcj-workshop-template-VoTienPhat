...
In the processing workflow, AWS Lambda AI Analyze reads the uploaded diagram from the Amazon S3 Input Bucket and prepares the input data to send to Amazon Bedrock. Amazon Bedrock identifies AWS services, connections, text notes, and architectural components from the diagram, then returns a structured architecture JSON. This architecture JSON becomes the central data source for the next analysis steps.
...
AWS Step Functions orchestrates the entire review process. First, Step Functions calls AWS Lambda AI Analyze to read the uploaded diagram from the S3 Input Bucket. Lambda AI Analyze prepares the input data and sends the diagram to Amazon Bedrock. Amazon Bedrock analyzes the diagram to identify AWS services, connections, text notes, and important architectural components, then returns a structured architecture JSON.

After receiving the architecture JSON, Lambda AI Analyze sends this data to AWS Lambda Cost Tool. Lambda Cost Tool uses the list of AWS services in the architecture JSON to estimate monthly costs. The result of this step is a cost estimation result, including detected services, usage assumptions, estimated costs, and components that may have a significant impact on the budget.
...
User → Amazon CloudFront → S3 Static Website React App → API Gateway → Lambda Upload Service → S3 Input Bucket → EventBridge → Step Functions → Lambda AI Analyze → Amazon Bedrock generates architecture JSON → Lambda Cost Tool estimates monthly cost → Amazon Bedrock performs comprehensive architecture review and cost optimization → Lambda PDF Generator → S3 Report Bucket + DynamoDB + SNS Email Notification.
...
8. AWS Step Functions orchestrates the review workflow and calls AWS Lambda AI Analyze.
9. AWS Lambda AI Analyze sends the uploaded diagram to Amazon Bedrock so Bedrock can identify AWS services, connections, text notes, and generate architecture JSON.
10. AWS Lambda AI Analyze sends the architecture JSON to AWS Lambda Cost Tool to estimate monthly costs.
...
![AI AWS Architecture Reviewer Architecture](fcj-workshop-template-VoTienPhat/images/2-Proposal/ai-aws-architecture-reviewer-proposal.jpg)
...
- **AWS Lambda AI Analyze**: Reads the uploaded architecture diagram from the Amazon S3 Input Bucket, prepares the input data, and sends the diagram to Amazon Bedrock to identify AWS services, connections, text notes, and generate architecture JSON.
...
- **AWS Lambda Cost Tool**: Receives the architecture JSON from Lambda AI Analyze, analyzes the AWS services detected in the diagram, and estimates monthly costs. The cost estimation result is sent back to Amazon Bedrock to support the comprehensive review and cost optimization step.
...
- AWS Lambda AI Analyze for diagram parsing.
...
- **Week 12: AI Workflow Integration, Reporting, Testing, and Completion**
  ...
  - Create Lambda AI Analyze to read uploaded diagrams from the S3 Input Bucket.
...
- Lambda AI Analyze reads uploaded diagrams and sends them to Amazon Bedrock.
